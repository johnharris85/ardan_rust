...
const CONFIG: bincode::config::Configuration<bincode::config::BigEndian> = bincode::config::standard().with_big_endian();
...
pub fn encode_response_v1(command: CollectorResponseV1) -> Vec<u8> {
    bincode::serde::encode_to_vec(command, CONFIG).unwrap()
}
...
pub fn encode_v1(command: &CollectorCommandV1) -> Vec<u8> {
    ...
    let payload_bytes = bincode::serde::encode_to_vec(command, CONFIG).unwrap();
...
pub fn decode_v1(bytes: &[u8]) -> (u32, CollectorCommandV1) {
    ...
    // Decode the payload
    (timestamp, bincode::serde::decode_from_slice(payload, CONFIG).unwrap().0)
}
